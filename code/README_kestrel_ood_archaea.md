# KESTREL — OOD Archaea Experiment
...
This script trains KESTREL on the BENDER dataset with **Archaea held out as the OOD kingdom**, then evaluates on both a held-out test set and the Archaea OOD sequences.
...
- `kingdom` — taxonomic kingdom (e.g. Bacteria, Fungi, Archaea)
...
```bash
python kestrel_ood_archaea.py \
    --data       bender_v1.csv \
    --out        kestrel_archaea_seed42/ \
    --seed       42 \
    --epochs     100 \
    --patience   15 \
    --lr         5e-4 \
...
| `kestrel_ood_r2.csv` | R² scores on OOD Archaea sequences |
